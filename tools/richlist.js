  ...
  asyncL.waterfall(
    [
      ...
      function (callback) {
        ...
        Block.aggregate([{ $match: { number: { $lte: toBlock, $gt: fromBlock } } }, { $group: { _id: "$miner" } }, { $project: { _id: 1 } }]).exec(
          (err, docs) => {
            if (err) {
              console.log(err);
              return;
          ...
          asyncL.eachSeries(
            ...
            (error) => {
              ...
              if (Object.keys(self.cached).length > ADDRESS_CACHE_MAX) {
                ...
                const sorted = Object.keys(self.cached).sort(
                  (a, b) => self.cached[b] - self.cached[a] // descend order
                );
                const newcached = {};
